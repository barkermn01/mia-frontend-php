        ...
        switch(type) {
            ...
            case 'secret':
                html = `
                    ...
                    <input type="password" id="secret_value" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Enter new secret value">
                    <p class="text-xs text-gray-500 mt-1">
                        <i class="fas fa-lock mr-1"></i>
                        Secret values are encrypted and masked. Enter a new value to replace the existing one.
                    </p>
                `;
                break;
    ...
    handleFormSubmit(e) {
        ...
        switch(type) {
            case 'text':
                ...
                break;
            case 'secret':
                value = document.getElementById('secret_value').value;
                if (!value || value.trim() === '') {
                    if (window.modalManager) {
                        window.modalManager.alert({
                            title: 'Validation Error',
                            message: 'Please enter a secret value',
                            type: 'warning'
                        });
                    } else {
